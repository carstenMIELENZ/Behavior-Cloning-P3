...
#
# Modified file:
# + added image pre-processing def process66x200 (lines 66-74)   
# + added/changed code for calling process66x200 (lines 92-94)
# + set throttle value ta 0.1
#  
#################################################################################
...
def telemetry(sid, data):
    if data:
        ...
        img = process66x200(image_array)   ## added 
        t_image_array = img[None, :, :, :] ## added
        steering_angle = float(model.predict(t_image_array, batch_size=1))
        #steering_angle = float(model.predict(image_array[None, :, :, :], batch_size=1))
        #throttle = controller.update(float(speed))
        throttle = 0.1
        print("sterring angle",steering_angle,"throttle", throttle)
